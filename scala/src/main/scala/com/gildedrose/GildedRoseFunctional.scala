...
object GildedRoseFunctional {
	...
	def updateQuality(items: Seq[ImmutableItem] = Seq.empty): Seq[ImmutableItem] = {
		...
		def rulesToApplyComp(name: String): ImmutableItem ⇒ ImmutableItem = name match {
			case n if n.startsWith("Backstage passes") ⇒ (in: ImmutableItem) => constrainQuality0to50(ageBackstage(decrementSellIn(in)))
			case n if n.startsWith("Aged Brie")        ⇒ (in: ImmutableItem) => constrainQuality0to50(ageBrie(decrementSellIn(in)))
			case n if n.startsWith("Sulfuras")         ⇒ (in: ImmutableItem) => ageSulfuras(in) // do no decrement sellIn or constrain
			case n if n.startsWith("Conjured")         ⇒ (in: ImmutableItem) => constrainQuality0to50(ageDownBy(2)(decrementSellIn(in)))
			case _                                     ⇒ (in: ImmutableItem) => constrainQuality0to50(ageDownBy(1)(decrementSellIn(in)))
		}

		val results2 = items.map(i => rulesToApplyComp(i.name)(i))

		items.map { i =>
			val toApply = rulesToApplyComp(i.name)
			toApply(i)
		}
	}
