...
object GildedRoseFunctional {
  ...
  def updateQuality(items: Seq[ImmutableItem] = Seq.empty): Seq[ImmutableItem] = {
    ...
    items.map { item ⇒
      rulesToApply(item.name).foldLeft(item) { case (item, transform) ⇒ transform(item) }
    }
  }
  ...
  private[gildedrose] def ageBrie(item: ImmutableItem): ImmutableItem = item.copy(quality = item.quality + 1)
  ...
  private[gildedrose] def ageBackstage(item: ImmutableItem): ImmutableItem = item.sellIn match {
    ...
    case in if in > 5  ⇒ item.copy(quality = item.quality + 3)
    case in if in > 0  ⇒ item.copy(quality = item.quality + 5)
